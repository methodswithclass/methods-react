...
var chooseElem = function ($$info) {
  if ($$info.id == 'gravity') {
    ...
    return (
      <div className="absolute left-100 width-500 height-100 top60 black-back white rounded20 border-white pointer">
        <div className="absolute center font-25">click here for code</div>
      </div>
    );
  }
};
  ...
  render() {
    ...
    return (
      <div className="relative width">
        <div
          className="relative width height-200 white-back cutoff noedge"
          id={getId('sep', info)}
        >
          <div
            className="relative width60 font-30 text-center center"
            id={getId('desc', info)}
          >
            {getHtml(info.description)}
          </div>
        </div>
